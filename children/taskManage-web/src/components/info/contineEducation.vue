      ...
      <div class="content">
        <el-form :model="learn" label-width="100px" :rules="learnRules" size="small" ref="learn" >
          ...
          <el-form-item label="学校" prop="school">
            <el-input  :maxlength="20" v-model="learn.school" placeholder="请输入学校"></el-input>
          </el-form-item>
          ...
          <el-form-item label="学科" prop="subject" placeholder="请输入学科" >
            <el-input  :maxlength="20" v-model="learn.subject"></el-input>
          </el-form-item>
          <el-form-item label="继教阶段"  placeholder="请输入继教阶段" >
            <el-input  :maxlength="20" v-model="learn.phase"></el-input>
          </el-form-item>
          <el-form-item label="证书编号" prop="certificateNo" placeholder="请输入学位证书号" >
            <el-input  :maxlength="20" v-model="learn.certificateNo"></el-input>
          </el-form-item>
  ...
  data () {
    return {
      ...
      learnRules: {
        ...
        subject: [
          { required: true, message: '请输入获学科', trigger: 'blur' }
        ],
        school: [
          { required: true, message: '请输入学校', trigger: 'blur' }
        ],
        // degree: [
        //   { required: true, message: '请输入继教阶段', trigger: 'blur' }
        // ],
        certificateNo: [
  ...
  methods: {
    ...
    addLearnIng (item) {
      this.$refs[item].validate(async (valid) => {
        if (valid) {
          await this.$store.dispatch('contineEducationSave', this.learn)
          this.learning = false
          this.educationList()
        } else {
          return false
    ...
    async delLearn (item) {
      this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
        ...
        await this.$store.dispatch('contineEducationDel', {
          id: item.id
        })
        this.learningList()
      })
    ...
    async educationList () {
      this.loading = true
      await this.$store.dispatch('contineEducation')
      if (this.$store.state.educationList) {
        this.checkState = this.$store.state.educationList.checkState
        this.learningList = this.$store.state.educationList.teacherContinuingEducation
        if (this.$store.state.educationList.teacherEditRecord) {
          // this.before = this.$store.state.educationList.teacherEditRecord
        }
      }
      this.loading = false
    },
