<template>
  <div class="proceHandle">
    ...
    <ul class="proceHandleContent" v-if="taskStatus * 1 === 1">
      ...
      <li v-if="!endFlag">
        <span>指派给：</span>
        <el-select v-model="toUserId" clearable size="small" @change="userChange" placeholder="请选择">
          <el-option
    ...
    <div class="proceTable">
      ...
      <el-table
        ...
        <el-table-column
          label="附件">
          <template slot-scope="scope">
            <div v-for=" imgList in scope.row.workLogAttachmentList " :key="imgList.id">
...
<script>
import { apiDelUploadFile, apiWorkHandle, apiUserlist, apiWorkLog } from '@/api/service'
export default {
  data () {
    return {
      taskTitle: '',
  ...
  methods: {
    ...
    // 选择指派人
    userChange () {
      let arr = this.userList.filter(item => item.id * 1 === this.toUserId * 1)
